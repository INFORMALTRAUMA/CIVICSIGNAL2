-- Master seed script for Civic Signal
...
-- Issue resolution + verifications
insert into issue_resolutions (issue_id, resolved_by, resolved_at, verification_count, resolution_note)
select i.id, 'seed-official-1', now(), 2, 'Resolved by field team.'
from issues i
where i.title = 'Streetlight outage on River Road'
  and not exists (
    select 1 from issue_resolutions r where r.issue_id = i.id
  );

insert into issue_verifications (issue_id, user_id)
select i.id, 'seed-user-4'
from issues i
where i.title = 'Streetlight outage on River Road'
  and not exists (
    select 1 from issue_verifications v where v.issue_id = i.id and v.user_id = 'seed-user-4'
  );

insert into issue_verifications (issue_id, user_id)
select i.id, 'seed-user-5'
from issues i
where i.title = 'Streetlight outage on River Road'
  and not exists (
    select 1 from issue_verifications v where v.issue_id = i.id and v.user_id = 'seed-user-5'
  );

-- Media placeholders (assumes you upload to storage manually)
insert into issue_media (issue_id, storage_bucket, storage_path, media_type, uploader_id)
select i.id, 'issue-media', 'seed/placeholder-1.jpg', 'image/jpeg', 'seed-user-1'
from issues i
where i.title = 'Open manhole near Central Market'
  and not exists (
    select 1 from issue_media m where m.issue_id = i.id and m.storage_path = 'seed/placeholder-1.jpg'
  );
